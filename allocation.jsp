...
<table>
  ...
  <%@page import= "java.io.*,java.util.*,java.sql.*,java.io.IOException,java.io.PrintWriter" %>
  <%
  try{  
		...
		ResultSet rs=stm.executeQuery("select * from student_basic");
		int i=0;
		while(rs.next())
		{
    ...
    <td><input type="image" style="outline: none;" value=<%=rs.getString(1)%> src="images/abs.png" width="30px" height="40px"  style="cursor: pointer;" data-toggle="modal" data-target="#myModal" class="abst" id="abs"/></td>
    <td id=<%=rs.getString(1)+"otable"%>>
    <select class="form-control" id=<%=rs.getString(1)+"o"%>>
    
      <option selected >Select guide</option>
       <%
	ResultSet staff=stm1.executeQuery("select * from staff_basic");
    while(staff.next())
	 {
      %>
  <option  class="staf" ><%=staff.getString(1)%></option>
  <% } %>
</select>
</td>
<td id=<%=rs.getString(1)+"oktable"%>>
 <button type="button" style="background: none;border: none; outline: none;" value=<%=rs.getString(1)+"$"+rs.getString(2)+"$"+rs.getString(3)+"$"+rs.getString(5)+"$"+rs.getString(4)%> class="ok" id=<%=rs.getString(1)+"ok" %>  data-toggle="modal"  >
          <span class="glyphicon glyphicon-ok"></span>
        </button> <button type="button"  style="background: none;border: none; outline: none;" class="cancel" value=<%=rs.getString(1) %> id=<%=rs.getString(1)+"cancel" %> data-toggle="modal" >
          <span class="glyphicon glyphicon-remove"></span>  
        </button>
<!-- <input type="checkbox" value=<%=rs.getString(1) %>  unchecked data-toggle="toggle" data-size="small" data-on="yes" data-off="No"   data-onstyle="success" data-offstyle="danger"> <button data-toggle="modal" data-target="#myModal1">YES</button> -->
...
<div class="container-fluid">
  ...
  <div class="row">
    <div class="col-sm-4" style="background-color:#2e363e;" id="header">
      <p><span>
 ...
 <%@page import= "java.io.*,java.util.*,java.sql.*,java.io.IOException,java.io.PrintWriter" %>
  ...
  try{  
		...
		PrintWriter prt= response.getWriter(); 
		Statement stm=con.createStatement();
		ResultSet rs=stm.executeQuery("select * from staff_basic");
		int i=0;
		while(rs.next())
		{
			%>
...
<div class="col-sm-4"  id="footer"></div>
